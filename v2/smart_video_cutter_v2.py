...
class SmartVideoCutter:
    def __init__(self, root):
        self.root = root
        self.root.title("✂️ Cắt Video Tự Động")
        ...
        tk.Button(root, text="📁 Thư mục nguồn", command=self.select_source_folder).grid(row=2, column=0)
        tk.Button(root, text="💾 Thư mục lưu", command=self.select_save_folder).grid(row=2, column=2)

        self.label_source = tk.Label(root, text="Nguồn: ")
        self.label_source.grid(row=3, column=0, columnspan=3)

        self.label_save = tk.Label(root, text="Lưu vào: ")
        self.label_save.grid(row=4, column=0, columnspan=3)

        tk.Button(root, text="🔹 Bắt đầu", command=self.set_start).grid(row=5, column=0)
        tk.Button(root, text="🔸 Kết thúc", command=self.set_end).grid(row=5, column=2)

        self.start_label = tk.Label(root, text="Start: -")
        self.start_label.grid(row=6, column=0)
        self.end_label = tk.Label(root, text="End: -")
        self.end_label.grid(row=6, column=2)

        tk.Button(root, text="✂️ Cắt & Tiếp theo", command=self.cut_and_next, bg="green", fg="white").grid(row=7, column=1)
    ...
    def select_source_folder(self):
        ...
        if folder:
            ...
            self.label_source.config(text=f"Nguồn: {folder}")
    ...
    def select_save_folder(self):
        ...
        if folder:
            ...
            self.label_save.config(text=f"Lưu vào: {folder}")
    ...
    def set_start(self):
        ...
        self.start_label.config(text=f"Start: {self.start_frame}")
        self.status.config(text=f"✅ Bắt đầu tại frame {self.start_frame}")

    def set_end(self):
        self.end_frame = self.current_frame
        self.end_label.config(text=f"End: {self.end_frame}")
        self.status.config(text=f"✅ Kết thúc tại frame {self.end_frame}")

    def cut_and_next(self):
        if not self.current_video_path or not self.save_folder:
            messagebox.showerror("Thiếu thông tin", "Hãy chọn thư mục nguồn và thư mục lưu.")
            return
